...
- **CI**（GitHub Actions）在 PR/push 时跑四个并行 Job：
  ...
  2. **单测 + 覆盖率** — `./gradlew test assembleDebug jacocoDebugUnitTestCoverageVerification`（行覆盖率门禁 ≥18% v5.6 起；v4.6 为 15%；v3.5 为 12%）
...
- v5 合入后为 **5.0.0**（versionCode 5，Room v11）；JaCoCo 门禁 **≥18%**；CI 三 job 生成 `google-services.json` stub；v5 smoke 见 [`docs/release-smoke-v5.md`](../release-smoke-v5.md)。
